...
export const generateFlashcards = async (text, count = 10) => {
    ...
    try {
        ...
        const flashcards = [];
        const cards = generateText_result.split('---').filter(x => x.trim());
...
export const generateQuiz = async (text, numQuestions = 10) => {
    ...
    try {
        ...
        for (const block of questionBlocks) {
            ...
            // Resolve "Q3" or "Q3: full text" to the actual option text
            if (correctAnswer.match(/^Q(\d+)/)) {
                const idx = parseInt(correctAnswer.match(/^Q(\d+)/)[1]) - 1;
...
export const generateSummary = async (text) => {
    ...
    try {
        return await generateText(promptQuestion);
    } catch (error) {
...
export const explainConcept = async (concept, context) => {
    ...
    try {
        ...
        console.error("Fail to explain the specific concept due to: " + error);
        throw error;
    }
};
